...
- [S] Legacy ledger records the per-object `DirTree.Find` lookup and linear scan.
- [O] Isolated benchmark: 4.95 seconds for 50,000 missing root lookups across 50,000 retained entries; benchmark source is not recorded.
...
Action: Reproduce the repeated-parent lookup benchmark on current `upstream/master`.
Done-When: Pinned source and benchmark confirm or invalidate the finding.
